...
class Jingdong():
    ...
    def __init__(self):
        ...
        for brand in text['brands']:
            url = 'https://list.jd.com/list.html?cat=9987,653,655&ev=exbrand_' + str(
                brand['id']) + '&sort=sort_rank_asc&trans=1&JL=3_' + quote(brand['name'])
                    ...
                    try:
                        ...
                        try:
                            ...
                            with open('products.csv', 'a') as f:  #
                                f.write(
                                    name + ',' + str(item_id) + ',' + params['skuName'] + ',' + params['price'] + ',' +
                                    commentMetas['goodRateShow'] + ',' + commentMetas['poorRateShow']
                                    + ',' + commentMetas['commentCount'] + ',' + commentMetas['goodCount'] + "," +
                                    commentMetas['generalCount'] + ',' + commentMetas['poorCount'])
                            ...
                            with open(name + '/' + str(item_id) + '_comments.csv', 'w') as f:
                                try:
                                    for comm in comments['goodComments']:
                                        ...
                                        f.write(
                                            str(comm['cmid']) + ',' + str(comm['guid']) + ',' + comm['nickname'] + ',' +
                                            comm['score'] + ',' + 'good,' + comm['creationTime'] + ',' + comm[
                                                'content'])
                                ...
                                try:
                                    for comm in comments['geneComments']:
                                        ...
                                        f.write(
                                            str(comm['cmid']) + ',' + str(comm['guid']) + ',' + comm['nickname'] + ',' +
                                            comm['score'] + ',' + 'gene,' + comm['creationTime'] + ',' + comm[
                                                'content'])
                                        if 'commentTags' in comm.keys():
                                            for commentTag in comm['commentTags']:
                                                f.write(',' + commentTag['name'])
                                ...
                                try:
                                    for comm in comments['badComments']:
                                        ...
                                        f.write(
                                            str(comm['cmid']) + ',' + str(comm['guid']) + ',' + comm['nickname'] + ',' +
                                            comm['score'] + ',' + 'bad,' + comm['creationTime'] + ',' + comm['content'])
                                        if 'commentTags' in comm.keys():
                                            for commentTag in comm['commentTags']:
                                                f.write(',' + commentTag['name'])
                                        f.write('\n')
                            ...
                            with open('item_exception.log', 'a') as f:
                                ...
                                f.write(str(datetime.datetime.now().strftime(
                                    '%Y-%m-%d %H:%M:%S')) + "," + self.path + "," + str(item_id) + "," + str(e) + "\n")
                                print(str(e))
                        ...
                        with open('item_error.log', 'a') as f:
                            # 格式：当前时间，所属类别，产品id，错误原因
                            f.write(str(datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')) + "," + item_url + str(
                                e) + "\n")
                            print(str(e))
    ...
    def getCommJson(self, productId, page=0, score=0):
        ...
        """
        # productPageComments
        commUrl = "http://sclub.jd.com/comment/skuProductPageComments.action?productId=" + str(
            productId) + "&score=" + str(score) + "&sortType=6&pageSize=10&isShadowSku=0&page=" + str(page)
    ...
    def getPrice(self, productId):  # 根据productId获取商品价格
        url = "https://p.3.cn/prices/mgets?type=1&area=1_72_2799_0&pdtk=&pduid=484377010&pdpin=&pin=null&pdbp=0&skuIds=J_" + str(
            productId) + "&ext=11000000&source=item-pc"
        ...
        for products in product_list:
            ...
            for item_id in products:
                try:
                    ...
                    name = soup.find("div", attrs={"class": "J-crumb-br"}).find("div", attrs={"class": "head"}).find(
                        'a').text  # 品牌
                    self.path = name
                    if not os.path.exists(self.path):
                        os.mkdir(self.path)
                    ...
                    try:
                        ...
                        with open('products_backup.csv', 'a') as f:  #
                            f.write(name + ',' + str(item_id) + ',' + params['skuName'] + ',' + params['price'] + ',' +
                                    commentMetas['goodRateShow'] + ',' + commentMetas['poorRateShow']
                                    + ',' + commentMetas['commentCount'] + ',' + commentMetas['goodCount'] + "," +
                                    commentMetas['generalCount'] + ',' + commentMetas['poorCount'])
                        ...
                        with open('item_exception.log', 'a') as f:
                            # 格式：当前时间，所属类别，产品id，错误原因
                            log = str(
                                datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')) + "," + self.path + "," + str(
                                item_id) + "," + str(e) + "\n"
                            f.write(log)
                        traceback.print_exc()
                    ...
                    with open('item_error.log', 'a') as f:
                        # 格式：当前时间，所属类别，产品id，错误原因
                        log = str(datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')) + "," + str(item_id) + str(
                            e) + "\n"
                        f.write(log)
                        print(log)
